...
* [getOrgCosts](#getorgcosts) - Get organization-wide cost summary
...
async function run() {
  const result = await linebundle.aiAgents.getOrgCosts();
...
import { aiAgentsGetOrgCosts } from "@linebundle-sdk/ts/funcs/ai-agents-get-org-costs.js";
...
async function run() {
  const res = await aiAgentsGetOrgCosts(linebundle);
  if (res.ok) {
    const { value: result } = res;
    console.log(result);
  } else {
    console.log("aiAgentsGetOrgCosts failed:", res.error);
  }
}
