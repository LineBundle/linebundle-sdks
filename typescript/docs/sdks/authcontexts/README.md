...
async function run() {
  const result = await linebundle.authContexts.get();
...
import { authContextsGet } from "@linebundle-sdk/ts/funcs/auth-contexts-get.js";
...
async function run() {
  const res = await authContextsGet(linebundle);
  if (res.ok) {
    const { value: result } = res;
    console.log(result);
  } else {
    console.log("authContextsGet failed:", res.error);
  }
}
