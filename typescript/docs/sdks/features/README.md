...
async function run() {
  const result = await linebundle.features.get();
...
import { featuresGet } from "@linebundle-sdk/ts/funcs/features-get.js";
...
async function run() {
  const res = await featuresGet(linebundle);
  if (res.ok) {
