...
* [handleOrgCreated](#handleorgcreated) - Webhook from Zitadel for organization creation
...
async function run() {
  const result = await linebundle.integrations.handleOrgCreated();
...
import { integrationsHandleOrgCreated } from "@linebundle-sdk/ts/funcs/integrations-handle-org-created.js";
...
async function run() {
  const res = await integrationsHandleOrgCreated(linebundle);
  if (res.ok) {
    const { value: result } = res;
    console.log(result);
  } else {
    console.log("integrationsHandleOrgCreated failed:", res.error);
  }
}
