...
* [handleRemoved](#handleremoved) - Webhook from Zitadel for user removal
...
async function run() {
  const result = await linebundle.integrationUser.handleRemoved();
...
import { integrationUserHandleRemoved } from "@linebundle-sdk/ts/funcs/integration-user-handle-removed.js";
...
async function run() {
  const res = await integrationUserHandleRemoved(linebundle);
  if (res.ok) {
    const { value: result } = res;
    console.log(result);
  } else {
    console.log("integrationUserHandleRemoved failed:", res.error);
  }
}
