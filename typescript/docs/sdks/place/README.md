...
* [publish](#publish) - Publish Place
* [archive](#archive) - Archive Place
...
async function run() {
  const result = await linebundle.place.publish({
    placeId: 313360,
  });
...
import { placePublish } from "@linebundle-sdk/ts/funcs/place-publish.js";
...
async function run() {
  const res = await placePublish(linebundle, {
    placeId: 313360,
  });
  if (res.ok) {
    ...
    console.log("placePublish failed:", res.error);
  }
}
...
async function run() {
  const result = await linebundle.place.archive({
    placeId: 75845,
  });
...
```typescript
import { LinebundleCore } from "@linebundle-sdk/ts/core.js";
import { placeArchive } from "@linebundle-sdk/ts/funcs/place-archive.js";
...
async function run() {
  const res = await placeArchive(linebundle, {
    placeId: 75845,
  });
  if (res.ok) {
    const { value: result } = res;
    ...
    console.log("placeArchive failed:", res.error);
  }
}
